...
def run(
    ...
    def schedule_say(coroutine: Coroutine[Any, Any, None]) -> None:
        """Run a narration coroutine on the stream event loop."""
        loop = stream_manager.asyncio_loop()
        if loop is None or not loop.is_running():
            logger.warning("Stream loop not ready; game narration skipped")
            return
        asyncio.run_coroutine_threadsafe(coroutine, loop)

    motion = GameMotion(movement_manager)
    narrator = GameNarrator(game, schedule_say, handler.say, motion=motion)
    game_server = GameServer(game, narrator)
    narrator.on_line = game_server.broadcast_narration
    narrator.attach()
    if effective_settings_app is not None:
        register_game_routes(effective_settings_app, game_server)
...
class ReachyImposterApp(ReachyMiniApp):  # type: ignore[misc]
