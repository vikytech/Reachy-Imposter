...
import reachy_imposter_app.console as console_mod
from reachy_imposter_app.config import HF_AVAILABLE_VOICES, config
...
def test_backend_startup_failure_is_recorded_without_raising(
    ...
    monkeypatch.setattr("reachy_imposter_app.console.apply_audio_startup_config", MagicMock())
...
def test_media_warmup_overlaps_audio_startup_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("reachy_imposter_app.console.has_hf_realtime_target", lambda: True)
    ...
    monkeypatch.setattr("reachy_imposter_app.console.asyncio.sleep", wait_for_audio_config)
    monkeypatch.setattr("reachy_imposter_app.console.apply_audio_startup_config", apply_audio_config)
...
def test_local_stream_persist_personality_clears_legacy_startup_env_overrides(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "reachy_imposter_app.config.set_custom_profile",
...
def test_local_stream_launch_waits_for_missing_hf_target_without_starting_media(
    ...
    monkeypatch.setattr("reachy_imposter_app.console.time.sleep", MagicMock(side_effect=KeyboardInterrupt))
