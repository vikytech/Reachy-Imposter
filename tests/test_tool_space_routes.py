...
def _configure_profiles(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Path, Path]:
    ...
    monkeypatch.setattr("reachy_imposter_app.profile_store.DEFAULT_PROFILES_DIRECTORY", profiles_root)
    monkeypatch.setattr(config, "REACHY_MINI_CUSTOM_PROFILE", None)
    monkeypatch.setattr(
        "reachy_imposter_app.tool_spaces.resolve_tool_space_sync",
...
def test_web_install_adds_global_inventory_without_enabling_a_profile(
    ...
    monkeypatch.setattr("reachy_imposter_app.tool_settings.initialize_tools", initialize_tools)
    restart_conversation = AsyncMock()
...
def test_locked_mode_exposes_inventory_but_rejects_tool_edits(
    ...
    instance_path, _ = _configure_profiles(tmp_path, monkeypatch)
    monkeypatch.setattr("reachy_imposter_app.tool_space_routes.LOCKED_PROFILE", "default")
    monkeypatch.setattr("reachy_imposter_app.profile_tool_routes.LOCKED_PROFILE", "default")
    client = _mount_rpc(instance_path, MagicMock(return_value=None), AsyncMock())
...
def test_active_profile_tool_update_restarts_a_running_conversation(
    ...
    instance_path, _ = _configure_profiles(tmp_path, monkeypatch)
    monkeypatch.setattr("reachy_imposter_app.tool_settings.initialize_tools", MagicMock())
...
def test_saved_tool_change_reports_success_when_runtime_reload_fails(
    ...
    instance_path, _ = _configure_profiles(tmp_path, monkeypatch)
    monkeypatch.setattr(
        "reachy_imposter_app.tool_settings.initialize_tools",
