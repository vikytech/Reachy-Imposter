...
import reachy_imposter_app.config as config_mod
from reachy_imposter_app.main import main
from reachy_imposter_app.mcp_client import RemoteToolSpec, RemoteMcpToolClient
...
from reachy_imposter_app.profile_store import write_profile
...
def test_tool_spaces_add_list_remove_round_trip(
    ...
    monkeypatch.setattr(
        "reachy_imposter_app.tool_spaces.HfApi.space_info",
        lambda self, slug, **kwargs: _mock_public_space_info(slug),
    )
    monkeypatch.setattr(
        "reachy_imposter_app.tool_spaces.RemoteMcpToolClient.list_tool_specs",
    ...
    monkeypatch.setattr("reachy_imposter_app.tool_spaces.resolve_tool_space_sync", resolve_tool_space)
...
def test_tool_spaces_add_installs_private_space_with_token(
    ...
    monkeypatch.setattr("reachy_imposter_app.tool_spaces.get_token", lambda: "hf_test_token")
    monkeypatch.setattr(
        "reachy_imposter_app.tool_spaces.RemoteMcpToolClient.list_tool_specs",
        _mock_list_tool_specs,
    )
...
def test_resolve_tool_space_sends_auth_only_to_private_spaces(
    ...
    monkeypatch.setattr("reachy_imposter_app.tool_spaces.get_token", lambda: login_token)
...
def test_tool_spaces_add_private_space_without_token_hints_at_auth(
    ...
    monkeypatch.setattr("reachy_imposter_app.tool_spaces.HfApi.space_info", _raise_not_found)
    monkeypatch.setattr(config_mod.config, "HF_TOKEN", None)
    monkeypatch.setattr("reachy_imposter_app.tool_spaces.get_token", lambda: None)
...
def _mock_add(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr("reachy_imposter_app.profile_store.DEFAULT_PROFILES_DIRECTORY", tmp_path)
    monkeypatch.setattr(
        "reachy_imposter_app.tool_spaces.HfApi.space_info",
        lambda self, slug, **kwargs: _mock_public_space_info(slug),
    )
...
def test_tool_space_install_profile_failure_leaves_manifest_unchanged(
    ...
    monkeypatch.setattr(config_mod.config, "PROFILES_DIRECTORY", tmp_path)
    monkeypatch.setattr(
        "reachy_imposter_app.tool_spaces.enable_profile_tools",
...
def test_tool_space_install_manifest_failure_rolls_back_profile(
    ...
    monkeypatch.setattr(config_mod.config, "PROFILES_DIRECTORY", tmp_path)
    monkeypatch.setattr(
        "reachy_imposter_app.tool_spaces.write_installed_tool_spaces",
